...
class Tie extends CI_Controller {
	...
	public function __construct()    //父类构造函数
	{
	    ...
	    $this->load->model('BBS_model');//BBS模块
	...
	public function ifHaveNewComment()
	{
		...
		$result = $this->BBS_model->ifHaveNewComment($TieID, $time);
		$json['tieID'] = $TieID;
		if($result == true)
	...
	public function ifHaveNewTie()
	{
		...
		$result = $this->BBS_model->ifHaveNewTie($time);
		if($result == true)
	...
	public function addComment()
	{
		...
		$json['tieID'] = $tieID;
		$result = $this->BBS_model->addComment($tieID, $comment, $userName, $userHead, $IP, $IParea, $time);
		if($result == false)
		{
	...
	public function getComment()
	{
		...
		//$page = "1";
		//$ifNeedCount = "yes";
		$result = $this->BBS_model->getComment($tieID, $page, $once);
		if($result == false)
		{
		...
		if($ifNeedCount == "yes")
			$json['count'] = $this->BBS_model->getCommentCount($tieID);
		$json['tieID'] = $tieID;
		$json['comment'] = $result;
		echo json_encode($json);
		return ;
	...
	public function getTieByID()
	{
		...
		$result = $this->BBS_model->getPostByID($ID);
		if($result == false)
		{
			$json['status'] = '600'; //未知的数据库错误
			echo json_encode($json);
	...
	public function getTieGreaterThanID()
	{
		...
		$res = $this->BBS_model->getPostAndGuaGreaterThanID($ID);
		if($res == null)
		{
	...
	public function getTieLessThanID()
	{
		...
		$res = $this->BBS_model->getPostAndGuaLessThanID($ID,$sum);
		if($res == null)
		{
	...
	public function getTieGreaterThanTime()
	{
		...
		$res = $this->BBS_model->getPostAndGuaGreaterThanTime($time);
		if($res == null)
		{
			$json['status'] = '201';//没有更新的帖子
			echo json_encode($json);
	...
	public function getTieLessThanTime()
	{
		...
		$res = $this->BBS_model->getPostAndGuaLessThanTime($time,$sum);
		if($res == null)
		{
			$json['status'] = '201';//没有更早的帖子
			echo json_encode($json);
	...
	public function getTie()
	{
		...
		$res = $this->BBS_model->getLatestPostAndGua($sum);
		if($res == null)
		{
			$json['status'] = '600';//未知的数据库错误
			echo json_encode($json);
	...
	public function post()
	{
		...
		$tieID = $this->BBS_model->newPost($title, $content, $userName, $userHead, $IP, $IParea, $time);
		if($tieID == false)
		{
			$json['status'] = '600';//未知的数据库错误
			echo json_encode($json);
...
*/
		...
		if($ifGua == "no")
		{
			$this->BBS_model->setIfGua($tieID, $ifGua);
		...
		else
		{
			...
			*/
			...
			$what= "0";
			$guaID = $this->BBS_model->newGua($year,$month,$day,$ganzhi_year,$ganzhi_month,$ganzhi_day,$age,$sex,$what,$nowGua,$nowBianGua,$nowGuaS,$nowGuaX,$nowGuaSB,$nowGuaXB,$nowGuaTotal,$nowGuaTotalB,$time,$userName,$IP,$IParea);
			if($guaID == false)
				$json['status'] = '501';
			else
				$this->BBS_model->setIfGua($tieID, $ifGua, $guaID);
		}
		echo json_encode($json);
		return ;
	}
