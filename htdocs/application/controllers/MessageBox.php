...
class MessageBox extends CI_Controller {
	...
	public function __construct()    //父类构造函数
	{
	    ...
	    $this->load->model('MessageBox_model');//站内信模块
	...
	public function ifHaveNewComment()
	{
		...
		$result = $this->MessageBox_model->ifHaveNewComment($TieID, $time);
		$json['tieID'] = $TieID;
		if($result == true)
	...
	public function ifHaveNewTie()
	{
		...
		$result = $this->MessageBox_model->ifHaveNewTie($time);
		if($result == true)
	...
	public function addComment()
	{
		...
		$json['tieID'] = $tieID;
		$result = $this->MessageBox_model->addComment($tieID, $comment, $userName, $userHead, $IP, $IParea, $time);
		if($result == false)
		{
	...
	public function getComment()
	{
		...
		//$page = "1";
		//$ifNeedCount = "yes";
		$result = $this->MessageBox_model->getComment($tieID, $page, $once);
		if($result == false)
		{
		...
		if($ifNeedCount == "yes")
			$json['count'] = $this->MessageBox_model->getCommentCount($tieID);
		$json['tieID'] = $tieID;
		$json['comment'] = $result;
		echo json_encode($json);
		return ;
	...
	public function getTieByID()
	{
		...
		$result = $this->MessageBox_model->getPostByID($ID);
		if($result == false)
		{
			$json['status'] = '600'; //未知的数据库错误
			echo json_encode($json);
	...
	public function getTieGreaterThanID()
	{
		...
		$res = $this->MessageBox_model->getPostAndGuaGreaterThanID($ID);
		if($res == null)
		{
	...
	public function getTieLessThanID()
	{
		...
		$res = $this->MessageBox_model->getPostAndGuaLessThanID($ID,$sum);
		if($res == null)
		{
	...
	public function getTieGreaterThanTime()
	{
		...
		$res = $this->MessageBox_model->getPostAndGuaGreaterThanTime($time);
		if($res == null)
		{
			$json['status'] = '201';//没有更新的帖子
			echo json_encode($json);
	...
	public function getTieLessThanTime()
	{
		...
		$res = $this->MessageBox_model->getPostAndGuaLessThanTime($time,$sum,$email);
		if($res == null)
		{
			$json['status'] = '201';//没有更早的帖子
			echo json_encode($json);
	...
	public function getMessage()
	{
		...
		$email = $this->input->post('email');
		$res = $this->MessageBox_model->getLatestPostAndGua($sum,$email);
		if($res == null)
		{
			$json['status'] = '600';//未知的数据库错误
			echo json_encode($json);
	...
	public function post()
	{
		...
		$tieID = $this->MessageBox_model->newPost($title, $content, $userName, $userHead, $IP, $IParea, $time,$hostEmail,$email);
		if($tieID == false)
		{
			$json['status'] = '600';//未知的数据库错误
			echo json_encode($json);
...
*/
		...
		$tieID = $this->MessageBox_model->newPost($title, $content, $userName, $userHead, $IP, $IParea, $time);
		...
		if($ifGua == "no")
		{
			$this->MessageBox_model->setIfGua($tieID, $ifGua);
		...
		else
		{
			...
			*/
			...
			$what= "0";
			$guaID = $this->MessageBox_model->newGua($year,$month,$day,$ganzhi_year,$ganzhi_month,$ganzhi_day,$age,$sex,$what,$nowGua,$nowBianGua,$nowGuaS,$nowGuaX,$nowGuaSB,$nowGuaXB,$nowGuaTotal,$nowGuaTotalB,$time,$userName,$IP,$IParea);
			if($guaID == false)
				$json['status'] = '501';
			else
				$this->MessageBox_model->setIfGua($tieID, $ifGua, $guaID);
		}
		echo json_encode($json);
		return ;
	}
