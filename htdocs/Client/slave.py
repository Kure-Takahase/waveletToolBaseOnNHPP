        ...
        while True:
            try:
                ...
                while True:
                    ...
                    CalculateTime = job[7]
                    start_time = job[8]
                    print("taskDistribution进程: 工作进程已空闲,从Master获取了"+taskName+" Task"+str(taskNum))
        ...
        while True:
            try:
                ...
                while True:
                    ...
                    i = 0
                    while i < listLen:
                        print("work进程: 清除之前加入的路径")
                        del sys.path[-1]
                        i = i + 1
                    fileList = []
                    # 若已加载过同名模块，则先清除同名模块
                    for job_file in job_files: 
                        if(job_file in sys.modules.keys()):
                            print("work进程: 已加载过"+job_file+"模块，先清除同名模块")
                            del sys.modules[job_file]
                    # 加入新路径
                    if uniqueNum not in fileList:
                    ...
                    # 提交结果给TaskResult队列
                    taskRes = (res_job,taskRes,resTuple)
                    resTask_queue.put(taskRes)
                    #print("work进程: "+str(len(taskRes)))
                    print("work进程: 完成了一个Task_"+str(taskNum))
            except Exception as e:
                print(e)
            else:
                pass
                ...
                while True:
                    try:
                        ...
                        while True:
                            ...
                            #print("taskResult 进程: "+str(len(resTaskResult)))
                            dispatched_jobs.put(resTaskResult)
                            print("taskResult 进程 返回了Task_"+str(resTaskResult[0][1])+"的结果")
                    except Exception as e:
                        resTask_queue.put(resTaskResult)
                        print("teskResult 进程崩溃!")
                        print("teskResult 进程崩溃!!")
                        print("teskResult 进程崩溃!!!")
                        print("teskResult 进程崩溃!!!!")
                        print(e)
