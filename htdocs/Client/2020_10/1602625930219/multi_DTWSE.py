...
# 设置每个工作簿内的列名元组
PredictionResult_3 = ()
PredictionStudy_3 = ('MSE1','MSE2','weightMethods','costMethods','costDataMethods','thresholdMethods','thresholdRules','dataTransforms','predictionIntervals','vares','compares','ifUseEstimation','ifCompareNewData','bandwidth','fullName')
PredictionResult_5 = ()
...
def taskFunction(dataTuple,paraTuple):
	...
	var = paraTuple[7]
	compare = paraTuple[8]
	ifUseEstimation = paraTuple[9]
	ifCompareNewData = paraTuple[10]
	# 预准备,分割数据
	...
	if(dataTransform == 'TranslationInvariant'):
		var = -1;
	result = DT_WSE.costPrediction(ahead_data,costMethod,costDataMethod,weightMethod,h,groupLength - predictionPoint,dataTransform,thresholdRule,thresholdMethod,var,compare,ifUseEstimation,ifCompareNewData);
	'''
	#加强版
...
def finialCust(excelFileName,taskResultTuples):
	...
	bookNames = ('weightMethods','costMethods','costDataMethods','thresholdMethods','thresholdRules','dataTransforms','vares','compares','ifUseEstimation','ifCompareNewData')
	paraLists = []
