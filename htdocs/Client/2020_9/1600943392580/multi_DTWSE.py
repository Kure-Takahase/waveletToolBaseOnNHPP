...
def finialCust(excelFileName,taskResultTuples):
	...
	for bookName in bookNames:
		...
		worksheet.write(j+1,2+col,np,cell_format)
		worksheet.write(j+1+raw,2+col,l[0]["mse"])
		worksheet.write(j+1+raw,1,l[0]["para"])
		worksheet.write(j+1+raw,0,dataName)
		for x in l:
			print(x)
			if(x["predictionPoint"] != nowPredictionPoint):
				...
				'''
				worksheet.write(j+1,2+col,np,cell_format)
				worksheet.write(j+1+raw,2+col,x["mse"])
				worksheet.write(j+1+raw,1,x["para"])
			elif(x["para"] != nowPara):
				nowPara = x["para"]
				raw = raw + 1
