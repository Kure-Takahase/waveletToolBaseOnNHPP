...
def multi_ReadParaFunction(ParaFile):
    ...
    # 读取参数
    MethodNames = ['weightMethods','costMethods','costDataMethods','thresholdMethods','thresholdRules','dataTransforms','predictionIntervals','vares']
    with open(ParaFile,'r') as f:
        ...
        while (i < len(data)) and (j < len(MethodNames)):
            ...
            k = 0
            while k < readLine:
                if(MethodNames[j] == 'predictionIntervals' or MethodNames[j] == 'vares'):
...
def SRATS_analysis(data_filename,para_filenames,predictionIntervals):
    ...
    for parameterList in parameterLists:
        ...
        for meanFunction in meanFunctions:
            ...
            print("参数："+str(parameterList[i][0])+" "+str(parameterList[i][1])+" "+str(parameterList[i][2]))
            old = 0
            while times <= groupLength:
                ...
                '''
                new = meanFunction(times,parameterList[i][0],parameterList[i][1],parameterList[i][2])
                Results[i].append(new - old)
                old = new
                '''
                #计算累积bug数
                Results[i].append(meanFunction(times,parameterList[i][0],parameterList[i][1],parameterList[i][2]));
...
SRATS_analysis(data_filename,para_filename,predictionInterval);
print("ok");
'''
'''
a = parameterList[1][0];
