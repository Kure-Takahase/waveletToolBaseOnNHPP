...
def costPrediction(data,costMethod,costDataMethod,weightMethod,h,times,dataTransform,thresholdRule,thresholdMethod,var,compare,ifUseEstimation,ifCompareNewData):
    ...
    i = 0;
    length = len(data);
    print(str(compare)+" "+str(ifUseEstimation)+" "+str(ifCompareNewData))
    while i < times:
        #获取数据
        if(costDataMethod == 'quota'):
            ...
            return false;
        # 跳过非法参数
        if(costMethod == 'denoise' and ifCompareNewData == 0) or (dataTransform == 'TranslationInvariant' and thresholdMethod == 'ut') or (dataTransform != 'TranslationInvariant' and thresholdMethod == 'ldt'):
            res = 0;
        elif(weightMethod != 'none' and compare != 0):
            res = 0;
        else:
            #进行预测
