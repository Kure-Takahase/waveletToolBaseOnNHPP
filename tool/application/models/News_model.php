...
class News_model extends CI_Model 
{
    ...
    {
    	$sql = "INSERT INTO news_comment (tieID, comment, userName, head, IP, IParea, time) VALUES ('".$tieID."', '".$comment."','".$userName."','".$userHead."','".$IP."','".$IParea."','".$time."')";
		$res = $this->db->query($sql);//增加新评论
		if($res == false)
			return false;
        $sql = "UPDATE news_tie SET time = '".$time."' WHERE id = '".$tieID."'";
    ...
    {
        $sql = "SELECT count(*) from news_comment WHERE tieID = '".$TieID."'";
    ...
    {
    	$start = ($page - 1) * $once ;
        $sql = "SELECT * from (SELECT * from news_comment WHERE tieID = '".$TieID."') as newTable ORDER BY time desc LIMIT ".$start.",".$once;
    ...
    {
    	$sql = "SELECT * from (SELECT * from news_tie WHERE id > '".$ID."') as newTable ORDER BY time desc";
    	$result = $this->db->query($sql)->result_array();
    ...
    {
    	$sql = "SELECT * from (SELECT * from news_tie WHERE id < '".$ID."') as newTable ORDER BY time desc LIMIT ".$sum."";
    	$result = $this->db->query($sql)->result_array();
    	foreach ($result as $key => $arr)
    	{
    		if($arr['ifgua'] == 'yes')
    ...
    {
        $sql = "SELECT * from (SELECT * from news_tie WHERE time > '".$time."') as newTable ORDER BY time desc";
        $result = $this->db->query($sql)->result_array();
        foreach ($result as $key => $arr) 
        {
            if($arr['ifgua'] == 'yes')
    ...
    {
        $sql = "SELECT * from (SELECT * from news_tie WHERE time < '".$time."') as newTable ORDER BY time desc LIMIT ".$sum."";
        $result = $this->db->query($sql)->result_array();
    ...
    public function setIfGua($id,$ifGua,$guaID = null)
    {
    	if($ifGua == "no")
    	{
    		$sql = "UPDATE news_tie SET ifgua = '".$ifGua."' WHERE id = '".$id."'";
    		return $this->db->query($sql);
    	}
    	else
    	{
    		$sql = "UPDATE news_tie SET ifgua = '".$ifGua."', guaID = '".$guaID."'  WHERE id = '".$id."'";
    		return $this->db->query($sql);
    	}
    }
    /**
     ...
     */
    public function newPost($title, $content, $user, $userHead, $IP, $IParea, $time)
    {
    	$sql = "INSERT INTO news_tie (title, content,user, head, IP,IParea,time,createTime) VALUES ('".$title."', '".$content."','".$user."','".$userHead."','".$IP."','".$IParea."','".$time."','".$time."')";
		$res = $this->db->query($sql);//增加新帖子
		if($res == false)
			return false;
		$sql = "SELECT id FROM news_tie WHERE title = '".$title."' AND time = '".$time."'";
		$result = $this->db->query($sql);//获取新帖子的ID
		if($result->result_array() == null)
			return false;
		return $result->result_array()[0]['id'];
    ...
    {
        $sql = "SELECT count(*) from (SELECT * from news_comment WHERE tieID = '".$TieID."') as newTable WHERE time > '".$time."'";
        $result = $this->db->query($sql)->result_array()[0]['count(*)'];
        if($result == "0")
            return false;
        return true;
    ...
    {
        $sql = "SELECT count(*) from news_tie WHERE time > '".$time."'";
        $result = $this->db->query($sql)->result_array()[0]['count(*)'];
        if($result == "0")
            return false;
        return true;
    ...
    {
    	$sql = "SELECT * from news_tie ORDER BY time desc LIMIT ".$sum."";
    	$result = $this->db->query($sql)->result_array();
    	return $result;
    }
    /**
    ...
    {
    	$sql = "SELECT * from news_tie where iftop != '1' ORDER BY time desc LIMIT ".$sum."";
    ...
    {
    	$sql = "SELECT * from news_tie WHERE id = ".$id."";
    	$result = $this->db->query($sql)->result_array();
    	if($result == null)
    		return false;
