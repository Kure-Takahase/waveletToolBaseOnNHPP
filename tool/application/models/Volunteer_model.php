...
class Volunteer_model extends CI_Model 
{
     ...
     */
    public function isHaveTieByEmail($email)
    {
        $sql = "SELECT * from host_list WHERE email = '".$email."'";
    ...
    {
    	$sql = "INSERT INTO comment (tieID, comment, userName, head, IP, IParea, time) VALUES ('".$tieID."', '".$comment."','".$userName."','".$userHead."','".$IP."','".$IParea."','".$time."')";
		$res = $this->db->query($sql);//增加新评论
		if($res == false)
			return false;
        $sql = "UPDATE tie SET time = '".$time."' WHERE id = '".$tieID."'";
    ...
    {
        $sql = "SELECT count(*) from comment WHERE tieID = '".$TieID."'";
    ...
    {
    	$start = ($page - 1) * $page;
        $sql = "SELECT * from (SELECT * from comment WHERE tieID = '".$TieID."') as newTable ORDER BY time desc LIMIT ".$start.",".$once;
    	$result = $this->db->query($sql)-> result_array();
    	if($result == null)
    		return false;
        foreach ($result as $key => $value) 
        {
            $result[$key]['head'] = base_url('/img/head/').$value['head'];
        }
    	return $result;
    }
    public function getPostAndGuaGreaterThanID($ID)
    {
    	$sql = "SELECT * from (SELECT * from tie WHERE id > '".$ID."') as newTable ORDER BY time desc";
    	$result = $this->db->query($sql)->result_array();
    ...
    {
    	$sql = "SELECT * from (SELECT * from tie WHERE id < '".$ID."') as newTable ORDER BY time desc LIMIT ".$sum."";
    	$result = $this->db->query($sql)->result_array();
    	foreach ($result as $key => $arr)
    	{
    		if($arr['ifgua'] == 'yes')
    ...
    {
        $sql = "SELECT * from (SELECT * from tie WHERE time > '".$time."') as newTable ORDER BY time desc";
        $result = $this->db->query($sql)->result_array();
        foreach ($result as $key => $arr) 
        {
            if($arr['ifgua'] == 'yes')
    ...
    public function getPostAndGuaLessThanTime($time, $sum)
    {
        $sql = "SELECT * from (SELECT * from tie WHERE time < '".$time."') as newTable ORDER BY time desc LIMIT ".$sum."";
        $result = $this->db->query($sql)->result_array();
    ...
    public function setIfGua($id,$ifGua,$guaID = null)
    {
    	if($ifGua == "no")
    	{
    		$sql = "UPDATE tie SET ifgua = '".$ifGua."' WHERE id = '".$id."'";
    		return $this->db->query($sql);
    	}
    	else
    	{
    		$sql = "UPDATE tie SET ifgua = '".$ifGua."', guaID = '".$guaID."'  WHERE id = '".$id."'";
    		return $this->db->query($sql);
    	}
    }
    /**
     ...
     */
    public function newPost($email,$host_data, $IP, $IParea, $time)
    {
        //var_dump($host_data);
    	$sql = "INSERT INTO host_list (email, hostName,country, province,city,age,drink_prefer,eat_prefer,is_smoke,is_cat,is_dog,is_handmade_article_room,is_animal_husbandry,is_farming_tree,is_farming_vegetable,is_farming_rice,is_restaurant,is_home_stay,introduction, IP,IParea,time) VALUES ('".$email."', '".$host_data['name']."', '".$host_data['country']."', '".$host_data['province']."', '".$host_data['city']."', '".$host_data['age']."', '".$host_data['drink_prefer']."', '".$host_data['eat_prefer']."', '".$host_data['is_smoke']."', '".$host_data['is_cat']."', '".$host_data['is_dog']."', '".$host_data['is_handmade_article_room']."', '".$host_data['is_animal_husbandry']."', '".$host_data['is_farming_tree']."', '".$host_data['is_farming_vegetable']."', '".$host_data['is_farming_rice']."', '".$host_data['is_restaurant']."', '".$host_data['is_home_stay']."', '".$host_data['introduction']."','".$IP."','".$IParea."','".$time."')";
		$res = $this->db->query($sql);//增加新帖子
		return $res;
    }
    public function updatePost($email,$host_data)
    {
        //var_dump($email);
        $sql = "UPDATE host_list SET hostName = '".$host_data['name']."' , country = '".$host_data['country']."' ,  province = '".$host_data['province']."' , city = '".$host_data['city']."' , age = '".$host_data['age']."' , drink_prefer = '".$host_data['drink_prefer']."' , eat_prefer = '".$host_data['eat_prefer']."' , is_smoke = '".$host_data['is_smoke']."' , is_cat = '".$host_data['is_cat']."' , is_dog = '".$host_data['is_dog']."' , is_handmade_article_room = '".$host_data['is_handmade_article_room']."' , is_animal_husbandry = '".$host_data['is_animal_husbandry']."' , is_farming_tree = '".$host_data['is_farming_tree']."' , is_farming_vegetable = '".$host_data['is_farming_vegetable']."' , is_farming_rice = '".$host_data['is_farming_vegetable']."' , is_restaurant = '".$host_data['is_restaurant']."' , is_home_stay = '".$host_data['is_home_stay']."' , introduction = '".$host_data['introduction']."' WHERE email = '".$email."'";
        //var_dump($sql);
        $res = $this->db->query($sql);//增加新帖子
        return $res;
    }
    ...
    {
        $sql = "SELECT count(*) from (SELECT * from comment WHERE tieID = '".$TieID."') as newTable WHERE time > '".$time."'";
        $result = $this->db->query($sql)->result_array()[0]['count(*)'];
        if($result == "0")
            return false;
        return true;
    ...
    {
        $sql = "SELECT count(*) from tie WHERE time > '".$time."'";
        $result = $this->db->query($sql)->result_array()[0]['count(*)'];
        if($result == "0")
            return false;
        return true;
     ...
     * @return array        帖子集
     */
    public function getLatestPostAndGua($sum)
    {
    	$sql = "SELECT * from host_list ORDER BY time desc LIMIT ".$sum."";
    	$result = $this->db->query($sql)->result_array();
    	return $result;
    }
    /**
    ...
    {
    	$sql = "SELECT * from tie where iftop != '1' ORDER BY time desc LIMIT ".$sum."";
    ...
    {
    	$sql = "SELECT * from tie WHERE id = ".$id."";
    	$result = $this->db->query($sql)->result_array();
    	if($result == null)
    		return false;
