...
class BBS_model extends CI_Model 
{
    ...
    {
    	$sql = "INSERT INTO tie (title, content,user, head, IP,IParea,time,createTime) VALUES ('".$title."', '".$content."','".$user."','".$userHead."','".$IP."','".$IParea."','".$time."','".$time."')";
		$res = $this->db->query($sql);//增加新帖子
		if($res == false)
			return false;
		$sql = "SELECT id FROM tie WHERE title = '".$title."' AND time = '".$time."'";
		$result = $this->db->query($sql);//获取新帖子的ID
		if($result->result_array() == null)
			return false;
		return $result->result_array()[0]['id'];
    ...
    {
    	$sql = "SELECT * from tie ORDER BY time desc LIMIT ".$sum."";
    	$result = $this->db->query($sql)->result_array();
    	return $result;
    }
    /**
