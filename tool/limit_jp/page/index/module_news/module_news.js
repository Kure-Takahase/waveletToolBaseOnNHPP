...
function addNews(parentElementID,newsData,isTail)
{
	...
	newsHTML += "<div class='layui-col-md12 remander_col' onclick='showNewsEr("+newsData.id+");'>";
		newsHTML += "<div class='title remander_title'>";
			...
			newsHTML += "<span class='remander_title'>"+ LimitControl.moduleObject("index","index_header").CNtoJP(newsData.title) +"</span>";
		newsHTML += "</div>";
		newsHTML += "<hr>";
		newsHTML += "<p class='the_content'>"+ LimitControl.moduleObject("index","index_header").CNtoJP(newsData.content)+"</p>";
		newsHTML += "<hr>";
