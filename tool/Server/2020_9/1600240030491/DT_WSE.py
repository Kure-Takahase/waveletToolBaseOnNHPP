...
def getMyThreshold(D):
    ...
    x0 = np.array([0])
    res = minimize(lostFunction, x0,(D,), method='nelder-mead',
                   options={'xatol': 1e-8, 'disp': False})
    #test start
    x1 = np.array([2])
    resu = minimize(lostFunction, x1,(D,), method='nelder-mead',
                   options={'xatol': 1e-8, 'disp': False})
    ...
    if(res.x[0] < 0):
        bounds=(0, 1)
        resul = minimize_scalar(lostFunction,None,bounds,D, method='bounded')
        res.x[0] = resul.x
...
def HFT(filename,thresholdMode,thresholdName,var):
    ...
    while j < len(C01s):
        i = 0
        while i < len(C01s[j]):
            if(C01s[j][i] != Ds2[j][0][i]):
                print("no! j=%d i=%d" % (j,i))
                x = 1;
            i = i + 1
        j = j + 1
    if(x == 0):
