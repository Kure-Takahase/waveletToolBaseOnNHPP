...
def finialCust(excelFileName,taskResultTuples):
	...
	for bookName in bookNames:
		...
		for taskResultTuple in taskResultTuples:
			...
			'''
			# 记录有多少个预测点
			if predictionInterval not in preList: 
				preList.append(predictionInterval)
			# 初始化数组
			if(paraList.has_key(paraTuple[i+2]) == False):
				paraList[ paraTuple[i+2] ] = dict()
			if( paraList[ paraTuple[i+2] ].has_key(predictionInterval) == False ):
				paraList[ paraTuple[i+2] ][ predictionInterval ] = mse2
				continue
			# 更新各个参数下不同预测点下最小的mse2
			if( paraList[ paraTuple[i+2] ][ predictionInterval ] > mse2 ):
				paraList[ paraTuple[i+2] ][ predictionInterval ] = mse2
			'''
		# 对数据进行排序
		print(taskResultTuples[0][1][i+2]+" 开始排序:")
