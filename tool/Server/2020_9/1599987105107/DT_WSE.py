...
def ThresholdForGroups(Ds,mode,ThresholdName):
    ...
    i = 0
    while(i < groupLength):
        list = ThresholdForGroup(Ds[i],mode,ThresholdName)
        lists.append(list)
        i = i + 1
    return lists

def ThresholdForGroup(GroupWaveletCoefficients,mode,ThresholdName):
    groupLength = len(GroupWaveletCoefficients)
    lists = []
    i = 1
...
def lostFunction(t,group):
    ...
    #print(groupO)
    #print(groupE)
    while i + 1 < len(group)/2:
        #print(i)
        e = t_group_E_[i] - groupO[i + 1]
        e = e*e
        o = t_group_O_[i] - groupE[i]
        o = o*o
        '''
        ...
        m = m + e + o
        i = i + 1
    e = t_group_E_[len(t_group_E_)-1] - groupO[0]
    e = e*e
    o = t_group_O_[len(t_group_O_)-1] - groupE[len(t_group_O_) - 1]
    o = o*o
    #print(e+o)
    m = m + e + o
    return m
...
def costPrediction(data,costMethod,costDataMethod,weightMethod,h,times,dataTransform,thresholdRule,thresholdMethod,var):
    ...
    while i < times:
        ...
        if(costDataMethod == 'quota'):
            ...
            return false;
        if(thresholdMethod == 'ut' and dataTransform == 'TranslationInvariant') or (thresholdMethod == 'ldt' and dataTransform != 'TranslationInvariant'):
            res = 0;
        else:
            #进行预测
            if(costMethod == 'origin'):
                res = getNextDayNum1(lists,dataTransform,thresholdRule,thresholdMethod,weightMethod,h,var);
            elif(costMethod == 'denoise'):
                res = getNextDayNum2(lists,dataTransform,thresholdRule,thresholdMethod,weightMethod,h,var);
            else:
                return false;
...
#获取之后N天的预测值,方法1
def getPrediction1(data,times,dataTransform,thresholdRule,thresholdMethod,weightMethod,h):
    originData = copy.deepcopy(data);
    i = 0;
    length = len(data);
    while i < times:
        #获取最近的length个数据
        lists = originData[i:length + i - 1];
        res = getNextDayNum1(lists,dataTransform,thresholdRule,thresholdMethod,weightMethod,h);
        originData.append(res);
        i = i + 1;
    return originData;
...
#获取下一天的预测值,方法1
def getNextDayNum1(data,dataTransform,thresholdRule,thresholdMethod,weightMethod,h,var):
...
#获取之后N天的预测值,方法2
def getPrediction2(data,times,dataTransform,thresholdRule,thresholdMethod,weightMethod,h):
    originData = copy.deepcopy(data);
    i = 0;
    length = len(data);
    while i < times:
        #获取最近的length个数据
        lists = originData[i:length + i - 1];
        res = getNextDayNum2(lists,dataTransform,thresholdRule,thresholdMethod,weightMethod,h);
        originData.append(res);
        i = i + 1;
    return originData;
...
#获取下一天的预测值,方法2
def getNextDayNum2(data,dataTransform,thresholdRule,thresholdMethod,weightMethod,h,var):
    data = copy.deepcopy(data);
    max_data = max(data) * 5;
    #print(max_data)
    bounds=(0, max_data);
...
#可选是否用核函数
def prediction_cost1(new_day,originDatas,dataTransform,thresholdRule,thresholdMethod,weightMethod,h,var):
...
#可选是否用核函数
def prediction_cost2(new_day,originDatas,dataTransform,thresholdRule,thresholdMethod,weightMethod,h,var):
    originDatas = copy.deepcopy(originDatas);
    #降噪
    if(dataTransform == 'Anscombe'):
        result1 = p_HAT(originDatas,thresholdRule,thresholdMethod,var)
